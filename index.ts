...
export default function (pi: ExtensionAPI) {
  pi.on("tool_result", async (event) => {
    ...
    const prompt = 
      ...
      `3.  **No Empty I/O:** A task cannot have 0 inputs AND 0 outputs. If a task truly does not read or write files, you **must** add a placeholder file (like \`package.json\`) to both sections to prevent the graph from becoming ambiguous and falling back to sequential mode.\n` +
