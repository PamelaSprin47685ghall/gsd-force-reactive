...
export default function (pi: ExtensionAPI) {
  pi.on("tool_result", async (event) => {
    ...
    const prompt = 
      ...
      `1. **Check Task Granularity:** Review the tasks. If any task is doing too much and could be split to take advantage of the ${maxParallel} max parallel workers, use \`edit\` or \`gsd_replan_slice\` to break it down.\n` +
      ...
      `**Action Required Now:**\n` +
      `Inspect the \`TXX-PLAN.md\` files now, break down tasks if needed, fix any that lack correct backtick-wrapped paths using \`edit\`, confirm the I/O chaining represents the true dependency order, and then reply saying "All task plans are verified and optimized for reactive execution." Only then will we proceed to the execution phase.`;
